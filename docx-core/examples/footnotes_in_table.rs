...
pub fn main() -> Result<(), DocxError> {
    ...
    let row1 = TableRow::new(vec![
        TableCell::new().add_paragraph(Paragraph::new().add_run(Run::new().add_text("普通文本"))),
        TableCell::new()
            .add_paragraph(Paragraph::new().add_run(Run::new().add_text("另一个单元格"))),
    ]);
    ...
    // 创建表格并设置网格
    let table = Table::new(vec![row1, row2]).set_grid(vec![2000, 2000]);
    ...
    Docx::new()
        ...
        .add_table(table)
        .add_paragraph(Paragraph::new().add_run(Run::new().add_text("表格后的段落")))
        .build()
        .pack(file)?;
