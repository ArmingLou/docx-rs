...
fn test_footnotes_in_table_are_collected() {
    // 创建脚注
    let footnote1 =
        Footnote::new().add_content(Paragraph::new().add_run(Run::new().add_text("表格脚注1")));

    let footnote2 =
        Footnote::new().add_content(Paragraph::new().add_run(Run::new().add_text("表格脚注2")));
    ...
    // 创建普通段落中的脚注
    let normal_footnote =
        Footnote::new().add_content(Paragraph::new().add_run(Run::new().add_text("普通脚注")));
    ...
    // 验证XML中包含我们期望的脚注内容
    assert!(
        footnotes_str.contains("普通脚注"),
        "脚注XML应该包含普通脚注内容"
    );
    assert!(
        footnotes_str.contains("表格脚注1"),
        "脚注XML应该包含表格脚注1内容"
    );
    assert!(
        footnotes_str.contains("表格脚注2"),
        "脚注XML应该包含表格脚注2内容"
    );

    // 验证XML结构正确
    assert!(
        footnotes_str.contains("<w:footnotes"),
        "应该包含footnotes根元素"
    );
    assert!(
        footnotes_str.contains("<w:footnote"),
        "应该包含footnote元素"
    );
